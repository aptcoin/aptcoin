...
#if defined(NDEBUG)
# error "Aptcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2bebe0197e9acb6afb8e0dded433c42204625304334fdabeed3d55286817a5e7");
static int64 aptCoinStartTime = 1408316143;
static int64 aptCoinTestnetStartTime = 1407450750;
...
const string strMessageMagic = "Aptcoin Signed Message:\n";
...
{
    // Aptcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned char GetNfactor(unsigned int hashPrevBlock, int64 blockTime)
{
    ...
    if ((aptCoinStartTime != -1) && (blockTime >= aptCoinStartTime))
    {
        ...
        // check if we're in the burn in period of testing
        nDiff -= ((elapsedTime < BURN_IN_PERIOD_SECONDS) ? B_STEP_DIFF : B_NORM_DIFF);
        nFactor = (hashPrevBlock % (nDiff + timeAdjustment)) + minNfactor;
    }
    /* printf("hashPrevBlock = %u, maxNfactor = %d, nFactor = %d, timeAdjustment[%lld] = %d\n", */
    /*        hashPrevBlock, (int)(maxNfactor + timeAdjustment), (int)nFactor, */
    /*        elapsedTime, (int)timeAdjustment); */
    return nFactor;
}

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = GET_SUBSIDY(50) * COIN;

    // halve block reward roughly every four years
    nSubsidy >>= (nHeight / FOUR_YEAR_BLOCK_HEIGHT);

    return (nSubsidy + nFees);
}

static const int64 nTargetTimespan = (20 * 5 * 60); // Aptcoin: every 20 blocks = ~1.5 hours
static const int64 nTargetSpacing = (5 * 60); // Aptcoin: 5 minute block target
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s (%u) progress=%f\n",
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aptcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Aptcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x428853007bc8d7a7b68ec8084203ba44ddaccc05394f79820daea8905468d069");
        aptCoinStartTime = aptCoinTestnetStartTime;
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = (fTestNet ?
                                    "AptCoin TestNet" :
                                    "Slashdot 08/17/2014 Are Altcoins Undermining Bitcoin's Credibility?");
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1408316142;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 394056503;
...
unsigned char pchMessageStart[4] = { 0xea, 0xc0, 0xa5, 0xca };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    printf("AptcoinMiner (Nfactor %d, hpb=%u):\n", GetNfactor(hpb, pblock->nTime), hpb);
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AptcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AptcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AptcoinMiner(CWallet *pwallet)
{
    printf("AptcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aptcoin-miner");
    ...
    try { loop {
       while (vNodes.empty())
        ...
        /* printf("Running AptcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(), */
    ...
    {
        printf("AptcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AptcoinMiner, pwallet));
}
